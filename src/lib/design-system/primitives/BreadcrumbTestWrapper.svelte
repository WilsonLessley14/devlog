<script lang="ts">
	import type { BreadcrumbItem, BreadcrumbTestWrapperProps } from './BreadcrumbProps';
	...
	let {
		...
		homeLabel = 'Home',
		class: className
	}: BreadcrumbTestWrapperProps = $props();
	...
	const breadcrumbItems = $derived(items ?? generateItemsFromPath(pathname));
